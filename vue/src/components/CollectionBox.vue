      ...
      <div class="coll-dialog-content">
        ...
        <div class="coll-table-body" v-else>
          ...
          <div class="table-row" v-for="item in collection.pageInfo.list" :key="item.id">
            ...
            <div class="table-cell time-cell">{{ formatTimeSecond(item.createdAt) }}</div>
...
const handleViewCollection = async (id) => {
  try {
    ...
    await nextTick()
    setTimeout(async () => {
      await scrollToSessionItem(session.id)
      await nextTick()
      setTimeout(() => {
        nextTick(() => {
